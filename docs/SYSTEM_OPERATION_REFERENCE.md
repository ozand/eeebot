...
8. Smoke gate: _run_smoke_tests() запускает ПОЛНЫЙ прогон `pytest tests/`
   (`-x -q --tb=short`, таймаут 60 сек) в изолированном чекауте — это
   реальный gate, а не проверка синтаксиса/импортов только изменённых
   файлов. Нет ни одного нового коммита на ветке цикла → gate пропускается.
   При FAIL запускается закрытый цикл repair-субагентов (issue #526),
   ограниченный REVISION_CAP_DEFAULT (3, R12); каждый repair-раунд коммитит
   на ТУ ЖЕ ветку цикла — интеграция (шаг 9) происходит один раз, после того
   как этот цикл repair завершится (успехом или исчерпанием попыток)
9. Integrate-to-main: при PASS gate _integrate_cycle_to_main() мержит HEAD
   ветки цикла в main (сброшенный на зафиксированный до старта цикла
   `main_sha_before`) и пушит `origin/main`. При FAIL (gate не прошёл, либо
   сам merge/push не удался) — main откатывается в точности к
   `main_sha_before` (git-проверяемо: `main_sha_before == main_sha_after` в
   результирующем артефакте), ветка цикла НЕ удаляется (хранится для
   расследования), а `_restore_to_main()` возвращает общий чекаут обратно на
   `main` с чистым деревом
10. _cleanup_cycle_branch() удаляет ветку цикла только после успешной
    интеграции (шаг 9, PASS)
...
субагент сломал что-то или тесты упали, main остаётся чистым; только
проверенные изменения интегрируются. Это даёт наблюдаемую границу
«сделано и проверено» на уровне git. Даже если субагент (вопреки инструкции)
сам выполнит `git push`, чекаут в момент его работы стоит на ветке цикла, а не
на main — паблишится в худшем случае только ветка цикла, `origin/main`
остаётся нетронутым; `origin/main` продвигает исключительно
`_integrate_cycle_to_main()`, и только после PASS gate.
...
который LiteLLM-прокси прозрачно маршрутизирует на реальный qwen-деплой.

История: до #641 вызов шёл через внешний бинарник `pi` (профиль
`NANOBOT_SUBAGENT_EXECUTOR=pi_dev`, провайдер `local_pi_cli`) с флагом
`--no-tools` — что функционально эквивалентно одному прямому вызову LiteLLM.
#641 убрал эту внешнюю зависимость целиком: рантайм больше не вызывает
`/usr/local/bin/pi` ни по одному пути; единственный встроенный исполнитель —
мост (`nanobot/runtime/bridge.py`) / `queued_request_terminalizer`.
