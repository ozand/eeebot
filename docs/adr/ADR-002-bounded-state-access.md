...
`nanobot.runtime.state_access` is the single stdlib-only foundation for bounded runtime-state reads. It exposes immutable `Window`, `Latest`, and `Sidecar` results. Readers never raise: `Window.status` is `complete`, `partial`, or `unavailable`; `Sidecar.status` is `present`, `absent`, `corrupt`, `oversize`, or `permission`.
