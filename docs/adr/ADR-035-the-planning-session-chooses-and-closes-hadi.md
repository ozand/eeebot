...
| auto-commit safety net for an interrupted executor (#1280/#1281) | `_auto_commit_uncommitted_work` commits a dead session's edits, and the normal gate may integrate them | the bridge's gate and integration path | **superseded** by `open_increment` `interrupted(supply\|defect\|kill)`. An unfinished session never integrates; its checkpointed work waits on its branch for keep, edit or delete. The auto-commit stays only as a checkpoint of residual edits on that branch, never as a route to integration |
...
| A new operator priority is shown to the planner, marked new, in the next cycle | `tests/test_agent_chooses.py::test_new_operator_priority_wakes_the_planner` | passing (#1942 B2) |
| A confirmed defect after a failed cycle reaches the planner first and a decline is named; three declines raise to the operator | `tests/test_agent_chooses.py::test_defect_urgency_survives_and_declines_are_visible` | passing (#1942 B2): `roles/planner.md`'s `declined` field (operator-approved wording) wired end-to-end in `tests/test_run_planning_session.py::test_planning_session_records_declined_defects_in_diary` |
...
| The planner's list keeps the demand trust order: new operator priority, then confirmed defect | `tests/test_agent_chooses.py::test_trust_order_new_priority_then_defect` | passing (#1942 B2); wired into `_run_planning_session`'s task via `demand.collect_demand` + `planner_candidates.render_candidates_block` |
...
| `rest` requires a wake condition naming an observable input and a deadline; without both it is `malformed`; rest never counts toward 3/6 | `tests/test_agent_chooses.py::test_rest_is_structured_and_outside_no_plan_counts` | passing (#1942 B2): `nanobot/runtime/planner_rest.py`, wired into `_run_planning_session` and `roles/planner.md` |
| The pre-check compares input versions only; unchanged + deadline not passed → no session and no repo preparation; any change, unreadable input or passed deadline → session | `tests/test_agent_chooses.py::test_precheck_tests_change_not_value` | passing (#1942 B2): gates both `_run_planning_session` and a new top-level check in `_main_impl_body` before repository preparation |
| Six rests in a row raise a review signal | `tests/test_agent_chooses.py::test_six_rests_raise_review_signal` | passing (#1942 B2) |
| Held ticks, `rest` decisions and `no_plan` are counted separately | `tests/test_agent_chooses.py::test_held_rest_and_no_plan_counted_apart` | passing (#1942 B2): three independent counters/state files (`planner_rest.py`, `no_plan_recovery.py`) |
| `rejected_duplicate` records the evidence sha, substitutes nothing, stays outside `no_plan`, and reaches the next session | `tests/test_agent_chooses.py::test_rejected_duplicate_is_recorded_and_fed_forward` | passing (#1942 B2): `nanobot/runtime/planner_dedup_evidence.py`, consumed into `_planner_task`. Applying the dedup gates to the PLAN's own chosen title/target (rather than substituting) is deferred to the Q1 selection-reorder still pending |
| A supply interruption carries the plan as `open_increment`, first and marked, to the next session; the harness never re-runs it by itself | `tests/test_agent_chooses.py::test_supply_interruption_carries_open_increment` | passing (#1942, architect resolution 2026-09-25): `nanobot/runtime/open_increment.py` |
| The supply hold: before the deadline no session, no repo preparation, no provider call; cooldown doubles to a 60-minute cap and resets on a completed call; after the deadline the session sees the open increment first | `tests/test_agent_chooses.py::test_supply_hold_uses_rest_snapshot_with_backoff` | passing (#1942, architect resolution 2026-09-25): the "session sees it first" half proven via `open_increment.render_open_increment_block`/`pending_open_increment`, not a full live-model round trip (no fixture for that exists yet in this suite) |
| Staged promotions and pending pushes survive repository preparation that now runs on every started cycle | `tests/test_agent_chooses.py::test_repo_preparation_preserves_staged_and_pending` | passing (#1942 B2): `bridge._prepare_repository_for_cycle`, called unconditionally (gated only by the rest pre-check) from a new top-level site in `_main_impl_body`; the old in-loop calls were left in place as a redundant-but-harmless safety net rather than removed |
...
| A session that ended in an error or left no terminal record never integrates, however many checkpoints its branch holds; supply → `interrupted(supply)`, our error → `interrupted(defect)` | `tests/test_agent_chooses.py::test_unfinished_session_never_integrates` | passing (B2 external-review fixes, #1962): session STATUS (`bridge._subagent_own_status`), not commit count, gates the closing-commit marker and the smoke/gate/integrate path; the checkpoints-exist+errored case gets its own classification (`open_increment.record_defect_interruption`/`record_supply_interruption`) and an explicit early return before either. `open_increment.clear_pending_on_integration` (new) is the ONLY thing that clears a pending increment on success -- `record_attempt_finished` (D2) only ever clears `running`, never `pending`, since a terminal ledger row can itself BE the write that sets `pending` (interrupted_supply/defect) in the same call |
| A `running` increment is recorded before the executor starts; a killed attempt (no post-execution bookkeeping) becomes `interrupted(kill)` with its branch at the next cycle start | `tests/test_agent_chooses.py::test_killed_attempt_becomes_interrupted_kill` | passing (B2 external-review fixes, #1962): `open_increment.record_attempt_started`/`check_running_for_kill`/`record_kill_interruption`; the check runs in `_main_impl` before the planning-session call (not inside it, since every `_main_impl`-level test stubs that function wholesale) |
| A kept increment's record survives a kill between the planning session and the executor | `tests/test_agent_chooses.py::test_keep_record_survives_kill_before_execution` | passing (B2 external-review fixes, #1962): `open_increment.resolve`'s `keep` branch no longer clears `pending` -- it only annotates `resumed_by`; only `record_attempt_finished` (a real terminal outcome) removes it |
...
| *delete* protects the branch with `refs/selfevo/inspect/<cycle_id>`, which pruning skips | `tests/test_agent_chooses.py::test_delete_keeps_inspection_ref` | passing (B2 external-review fixes, #1962): `open_increment.resolve(..., selfevo_repo=...)` pins the branch tip via `update-ref` before clearing `pending`; `_prune_stale_cycle_branches` never queries that ref namespace, so it survives the branch itself being pruned |
| A pending increment with a missing or invalid `open_increment_decision` is an invalid plan (`no_plan` family), never silent acceptance of a new plan | `tests/test_agent_chooses.py::test_missing_open_increment_decision_is_no_plan` | passing (B2 external-review fixes, external review finding #8, #1962): recorded as `malformed` (planner family in `no_plan_recovery`), plan returned as `None` -- drives the REAL `_run_planning_session` (only `SubagentManager` faked), not a stub |
| `stopped` blocks session start until the operator resumes manually; `minimal_mode` actually selects a smaller planner context | `tests/test_agent_chooses.py::test_stopped_and_minimal_mode_are_enforced` | passing (B2 external-review fixes, external review finding #9, #1962): `stopped` gates the `_run_planning_session` call in `_main_impl` (not inside it -- every test here stubs that function wholesale); `minimal_mode` skips the candidates/dedup-evidence blocks in favor of a short marker |
| A terminal supplier error in planner telemetry is classified before JSON parsing: family `supply`, not `planner` | `tests/test_agent_chooses.py::test_planner_supplier_error_classified_as_supply` | passing (B2 external-review fixes, external review finding #10, #1962): `status=='error'` checked and classified (`_classify_llm_error`, reused from the executor's own path) before `_parse_planner_final_response` runs; a genuine match records the new `supplier_error` outcome (`CAUSE_FAMILY` -> supply) |
| The supply-interruption streak resets on ANY successful planner call, including `rest` | `tests/test_agent_chooses.py::test_planner_rest_resets_supply_streak` | passing (B2 external-review fixes, small item, #1962) |
| `planner_rest`'s version snapshot checks the returncode of `git rev-parse`, not just stdout | `tests/test_agent_chooses.py::test_planner_rest_snapshot_checks_returncode` | passing (B2 external-review fixes, small item, #1962) |
| `_recent_commits_with_paths` returns unknown (`None`) on a git failure, never `[]` | `tests/test_agent_chooses.py::test_recent_commits_with_paths_returns_unknown_on_git_failure` | passing (B2 external-review fixes, small item, #1962): the caller (`_recent_activity_context`) now distinguishes the two with its own line |
| `scripts/change_shape.py` uses the shared exclusion constant | `tests/test_agent_chooses.py::test_change_shape_excludes_checkpoints_via_shared_constant` | passing (B2 external-review fixes, small item, #1962): `classify_integrated_commits` now excludes residual/checkpoint commits via `commit_markers.ARTIFICIAL_COMMIT_GREP_PATTERNS`, same as every other "is this already done" reader |
| `scripts/prototype_already_implemented.py`'s offline evidence builder includes checkpoint commits, not just `selfevo: auto-commit` subjects | -- | **Not fixed, note only** (small item, #1962): an offline prototype, no established live rejection gate calls it (external review's own assessment); left as a known gap for whoever wires it into a live gate |
| On `keep`, the planner confirms/edits the previous plan instead of re-planning from scratch | `tests/test_agent_chooses.py::test_keep_confirm_prompt_and_task_reuse_previous_plan` and `test_keep_edit_diary_records_edit_of_previous_plan` | passing (D11, architect resolution 2026-09-26 from the #1903 planning-cost measurement, #1962): `plan_action` (`confirm`/`edit`) added to the open-increment block; confirm substitutes the pending increment's own stored plan text before the mandatory-`plan`-field check runs, reusing it verbatim for both the executor's task and the new diary entry at zero extra planner cost; edit still requires and uses a revised `plan`, recorded in the diary as an edit of the same plan |
