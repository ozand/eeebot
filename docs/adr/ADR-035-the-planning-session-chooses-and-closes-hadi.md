...
| A confirmed defect after a failed cycle reaches the planner first and a decline is named; three declines raise to the operator | `tests/test_agent_chooses.py::test_defect_urgency_survives_and_declines_are_visible` | partial (#1942 B2): decline-counter/escalation backend passing; the planner's OUTPUT side (`roles/planner.md` + parsed-response schema for `declined`) is not wired -- prompt wording needs sign-off before commit |
