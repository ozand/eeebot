...
- R35 (issue #750). In addition to R32/R33's exact-title/keyword checks, the
  ...
  from `<state_dir>/subagents/results/*.json` (hypothesis titles were
  indexed too until #1219 retired that corpus; the backlog snapshot that fed
  it is gone since #1356). A `script`-kind FTS candidate is
...
- R36 (issue #751). Every proposal SHALL name what goal it serves, and the
  ...
  - **Hypothesis-backlog reader.** `nanobot/runtime/hypothesis_backlog.py`
    ...
    from the strategist's `<state_dir>/hypotheses/durable.json`. (Originally
    `<state_dir>/hypotheses/backlog.json` primary and
    `<state_dir>/research/hypotheses.json` secondary; the research feed lost
    its writer with the planner and left in #1219; the backlog snapshot only
    ever held the request the loop had just written for itself and was
    retired with its writer in #1356.) Candidates
    ...
    the source file itself, which is regenerated by its own writer and so
    cannot hold cross-cycle status without a read-modify-write change to
    that writer.
