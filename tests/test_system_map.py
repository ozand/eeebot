...
# ─── _map_has_foreign_generator ────────────────────────────────────────────


class TestMapHasForeignGenerator:
    def test_absent_or_empty_is_not_foreign(self):
        assert system_map._map_has_foreign_generator("") is False
        assert system_map._map_has_foreign_generator("   \n") is False

    def test_our_own_map_is_not_foreign(self):
        our_map = (
            "# SYSTEM MAP\n\n"
            f"{system_map._GENERATED_NOTE}\n\n"
            "## Inventory\n\n- scripts/a.py — a\n"
        )
        assert system_map._map_has_foreign_generator(our_map) is False

    def test_foreign_map_without_marker_is_foreign(self):
        foreign = (
            "# System Map\n\n"
            "It is automatically generated by `scripts/generate_system_map.py`.\n\n"
            "## Scripts by Theme\n\n### Deployment\n\n- deploy_release.py — ships releases\n"
        )
        assert system_map._map_has_foreign_generator(foreign) is True

    def test_hand_edited_map_without_marker_is_foreign(self):
        assert system_map._map_has_foreign_generator("# Notes\n\nsome hand-written text\n") is True
...
class TestUpdateSystemMap:
    ...
    def test_foreign_generator_map_deferred_untouched(self, tmp_path):
        ...
        the instance's own scripts/generate_system_map.py) must never be
        clobbered — update_system_map defers entirely, even after HEAD moves.
        """
        repo = _git_repo(tmp_path)
        state_dir = tmp_path / "state"
        (repo / "docs").mkdir()
        ...
        (repo / "docs" / "SYSTEM_MAP.md").write_text(foreign_content, encoding="utf-8")

        # Move HEAD so Gate 1 alone would not short-circuit.
        _write_script(repo, "scripts/new_script.py", '"""New."""\n')
        _commit_all(repo, "feat: add new_script.py")

        wrote = system_map.update_system_map(repo, state_dir)

        assert wrote is False
        assert (repo / "docs" / "SYSTEM_MAP.md").read_text(encoding="utf-8") == foreign_content
        assert not (state_dir / "system_map" / "watermark.json").exists()

    def test_our_marker_map_regenerates_normally(self, tmp_path):
        """A map bearing our own marker (i.e. previously written by this
        module) is not foreign — normal watermark-gated regeneration applies.
        """
        repo = _git_repo(tmp_path)
        state_dir = tmp_path / "state"
        assert system_map.update_system_map(repo, state_dir) is True
