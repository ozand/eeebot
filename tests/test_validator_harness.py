...
usage-evidence writer integration, the total per-invocation time budget,
and fail-open behavior on corrupt state / a missing repo / no git.
"""
...
class TestUsageEvidenceIntegration:
    ...
    def test_failing_script_still_records_usage_evidence(self, tmp_path):
        """A validator that ran and failed was still genuinely exercised —
        usage evidence is recorded regardless of exit code."""
        repo = _init_repo(tmp_path)
        _add_script(repo, "check_fail.py", "import sys\nsys.exit(3)\n", days_ago=2)
        state_dir = _state_dir(tmp_path)
        validator_harness.run_validator_harness(state_dir, repo)
