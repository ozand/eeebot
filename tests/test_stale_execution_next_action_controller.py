...
def _next_action_payload(task: dict[str, object], queue_path: Path, incident_path: Path, next_action_path: Path) -> dict[str, object]:
    return {
        ...
        'watchdog_result': {
            ...
            'age': '14h29m59s',
            'recommended_next_action': 'Treat this as a stale-execution incident: check the live executor, confirm whether the task is still running, and either record the terminal result or re-dispatch the bounded slice.',
