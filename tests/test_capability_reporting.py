...
def test_runtime_state_exposes_capabilities_snapshot(tmp_path: Path):
    ...
    (state / 'experiments' / 'latest.json').write_text(json.dumps({'budget': {'max_requests': 1, 'max_tool_calls': 8, 'max_timeout_seconds': 10}, 'budget_used': {'requests': 1, 'tool_calls': 8, 'elapsed_seconds': 10}}), encoding='utf-8')
