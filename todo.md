...
- [ ] 10. Lane-selection stuck in review/bookkeeping → no material progress — INVESTIGATED 2026-06-23, no code change shipped
  - Live evidence (eeepc, post #8/#9 deploy): across 2 cycles the loop selected `exploit-successful-improvement-path` / `inspect-pass-streak` / `subagent-verify-materialized-improvement` lanes → `PASS/keep` but `changed_files=None`.
  - Investigation (coordinator.py `_derive_feedback_decision`): the system ALREADY has a designed PASS-streak → materialize path (the `inspect-pass-streak → materialize-pass-streak-improvement` ladder + the `ambition_underutilization_reasons` → MATERIALIZE escalation at the `elif ambition_underutilization_reasons:` branch). Review cycles before materializing are the intended cadence, not a bug.
  - REJECTED approach: a "force materialize after N change-free PASS cycles" trigger (added `_change_free_pass_streak` + escalation reason). It conflicts with the intended pass-streak ladder and **broke 15 existing tests** (`test_selector_pass_streak`, `test_self_improvement_followthrough`, …). Reverted. Lesson: do not blanket-override the designed lane cadence on a change-free signal.
  - Open (only if a persistent stall is confirmed over MORE cycles on the now-correct deploy): the narrow, lower-risk fix is the **carried-candidate dedup precedence** — `combined = [*carried, *inferred, *generated]` keep-first lets a prior generic synthesize/materialize candidate shadow the fresh insight-derived one (#8). Fix = prefer the `derived_from_insight` candidate for those synthesized task_ids. Verify it is the live cause before shipping.
  - Recommendation: the deploy already put executor-gate (#170) + #8/#9 into live execution for the first time since 2026-06-15; observe several more cycles before any behavioral change. 2 cycles is not proof of a permanent stall.
