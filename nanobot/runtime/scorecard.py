...
def _loop_section(
    ...
    idle_rows = 0
    outcome_rows = 0
    # #1411: cycles whose terminal outcome row carries lane=="fallback" — a
    # same-cycle proposal generated by the fallback lane, not the demand-
    # driven proposer. One outcome row per cycle (record_cycle_outcome's own
    ...
    return {
        ...
        # it earned its cost without a separate query. `fallback_share` is
        # scoped to cycleish (idle + outcome rows), matching `idle_share`'s
        # denominator — both describe "what fraction of this cycle's work
        # was NOT the ordinary demand-driven path".
