...
#
# #929: "output" (pre-existing member) requires causal content binding, not
# mtime freshness alone. Runtime-churned bookkeeping targets (ledger/cycles,
# scorecard/latest, validator_harness/last_runs, usage and other runtime-owned files)
# do not qualify as output artifacts unless independent causal evidence exists.
# Genuinely produced outputs (e.g. docs, generated maps/reports, state artifacts)
# qualify when bound to the producing script and not pre-dating script creation.
# The gate is implemented in ``_output_signal`` and is fail-open (if git creation
# is unavailable the artifact is accepted as-is).
HARNESS_SIGNALS: frozenset[str] = frozenset({"pycache", "output", "benchmark", "reference", "harness_run"})

# Runtime-owned bookkeeping targets that churn during normal loop operation and must
# not qualify as output artifacts unless independent causal evidence exists (#929).
_RUNTIME_CHURNED_PREFIXES: tuple[str, ...] = (
    "state/ledger/",
    "state/scorecard/",
    "state/validator_harness/",
    "state/usage/",
    "state/goals/",
    "state/demand/",
    "state/heldout/",
...
_RUNTIME_CHURNED_EXACT: frozenset[str] = frozenset({
    "state/ledger/cycles.jsonl",
    "state/scorecard/latest.json",
    "state/scorecard/history.jsonl",
    "state/validator_harness/last_runs.jsonl",
    "state/usage/last_used.json",
    "state/goals/goal_text.txt",
})
...
def _is_runtime_churned_target(token: str) -> bool:
    """Return True if token targets runtime-churned bookkeeping paths."""
    norm = token.replace("\\", "/").strip("./")
    if norm in _RUNTIME_CHURNED_EXACT:
        return True
    return any(norm.startswith(prefix) for prefix in _RUNTIME_CHURNED_PREFIXES)
...
def _output_signal(
    ...
    *,
    ...
    #929 causal binding gate: output signal must not be earned by mtime
    freshness alone. Runtime-churned bookkeeping targets (ledger/cycles,
    scorecard/latest, validator_harness/last_runs, usage and other runtime-owned
    state files) do not qualify as output artifacts unless independent causal
    evidence exists. Genuinely produced or consumed output artifacts qualify
    when bound to the producing script and not older than the script's git
    creation date.

    Fail-open: if ``_git_creation_iso`` returns ``None`` (untracked file, git
    unavailable), the creation date gate is skipped while runtime-churned
    exclusions still protect against bookkeeping artifacts.
    """
    try:
        ...
        for token in header_tokens:
            ...
            # #929: output paths are repository/state-relative only. Reject
            # absolute, drive-qualified, and traversal paths before joining;
            # Path(base) / absolute_rhs otherwise discards the trusted base.
            token_parts = norm_token.split("/")
            if (
                not norm_token
                or norm_token.startswith("/")
                or re.match(r"^[A-Za-z]:", norm_token)
                or ".." in token_parts
            ):
                continue
            # #929: Reject runtime-churned bookkeeping artifacts
            if _is_runtime_churned_target(norm_token):
                continue

            if norm_token.startswith("state/"):
                candidate = Path(state_dir) / norm_token[len("state/"):]
            else:
                candidate = Path(selfevo_repo) / norm_token
