...
def _push_main_or_report(
    ...
    detail = ''
    try:
        push = _sp_push.run(git + ['push', 'origin', refspec], capture_output=True, text=True, timeout=_GIT_SYNC_TIMEOUT)
    ...
    print(f'{label}: push origin {refspec} rejected ({detail})')
...
async def _main_impl_body():
    ...
    while True:
        ...
        # resolver (no path constructed here). ADR-034 rule 5: the
        # operator's own "Current priority targets" section
        # (state/goals/goal_text.json, via resolve_operator_priorities_text
        # — never a second path to it) is folded in ALONGSIDE the charter,
        # never as a charter substitute — the deleted legacy chain here
        # used to read RELEASE_ROOT/host/eeepc/etc/goal_text.json (a path
        # that exists on no host, #1699 census) and, when the charter was
        # present, dropped the operator's priorities from the executor
        # mission entirely (rule 5 requires the executor to see them).
        # Derived priorities (derived_priorities.json) are always folded in
        # next, by merged_goal_text.
        try:
        ...
        _priority_marker_idx = _priorities_text.find('Current priority targets:')
        _priority_section = _priorities_text[_priority_marker_idx:].strip() if _priority_marker_idx != -1 else ''
        if _charter:
            _base_goal_text = f"{_charter.rstrip()}\n\n{_priority_section}" if _priority_section else _charter
        else:
            _base_goal_text = _priority_section or goal_id
        try:
            from nanobot.runtime.goal_review import merged_goal_text
...
def _lesson_candidate_parts(
    artifact_data: dict | None,
    *,
    backlog_title: str = "",
    cycle_id: str = "",
) -> tuple[str, str, list[str]]:
    """Extract a condition/action pair for the reflector recurrence pool.

    The bridge may decide that an integrated cycle has meaningful delta
    evidence, but the condition and recommendation must remain separate. Read
    the same nested lesson containers as the existing insight/delta helpers;
    never substitute a generic improvement statement for the condition.
    """
    if not isinstance(artifact_data, dict):
    ...
    for container in containers:
        for key in ("problem", "condition", "observation"):
    ...
    if not condition:
        for container in containers:
            value = container.get("hypothesis")
            if isinstance(value, str) and value.strip():
                condition = value.strip()
                break

    detail = str(
        _extract_reflector_solution(artifact_data)
        or _extract_meaningful_insight(artifact_data)
        or ""
    ).strip()
    raw_evidence = artifact_data.get("evidence") or [cycle_id]
    if isinstance(raw_evidence, str):
        raw_evidence = [raw_evidence]
    evidence = [str(item) for item in raw_evidence if isinstance(item, (str, int, float)) and str(item).strip()]
    return condition, detail, evidence[:8] or ([cycle_id] if cycle_id else [])
