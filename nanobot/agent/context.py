...
{skills_summary}""" if skills_summary else "")
    ...
    def _load_priorities_block(self) -> str:
        ...
        the executor's own operator-priorities + derived-priorities section.
        ...
        the shared renderer replaces it whole with an ``unavailable``/
        ``oversize`` marker rather than truncating (ADR-034 rule 3,
        extended to this rendered block).
    ...
    def _build_loop_system_prompt(
        ...
        *,
        ...
        # ADR-034 rule 5 (#1940, A4): operator + derived priorities, right
        # after the charter -- "operator first" -- and entirely outside the
        # release pool those ontology blocks just drew from (own budget,
