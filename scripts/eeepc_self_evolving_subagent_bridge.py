...
def build_task(req: dict, goal_text: str, report_source: str,
    ...
    lines += [
        '## Your instructions — you are an IMPLEMENTER, not a reviewer',
        'Your session is a FAILURE unless it ends with a git commit that edits a real file.',
        'Reading/greping without writing is NOT progress.',
        '',
        '1. Orient briefly: AT MOST 2-3 read_file/exec calls to locate the target file. Then STOP exploring.',
        '2. Make the change NOW with write_file or edit_file on a concrete file',
        '   (prefer surfaces/, scripts/, nanobot/, memory/). git pull first if needed.',
        '3. Verify: exec("python3 -c \'import <module>; print(\\"ok\\")\'")  (pytest is not installed).',
        '4. COMMIT — MANDATORY: exec("git add -A && git commit -m \'<type>: <what>\'").',
        '   outcome="completed" is allowed ONLY if git commit actually succeeded; else use "blocked".',
        '5. If the whole task is too large, implement the SMALLEST concrete slice that imports/compiles',
        '   and commit THAT — a small real commit beats a perfect report. Do NOT finish with zero edits.',
        '6. After a successful commit, update memory/MEMORY.md:',
        ...
        'You have up to 15 iterations — spend AT MOST 3 on orientation, the rest on editing + committing.',
        'A session that ends with no write_file/edit_file call is a FAILURE; commit at least one real edit.',
    ]
